...
export function NetworkMonitor() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: 'Unable to establish database connection. Using simulated data.',
        icon: 'warning',
        background: '#1e293b',
        color: '#fff',
  ...
  return (
    <div className="relative overflow-hidden transition-all duration-300 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 p-6 rounded-xl">
      ...
      {nagiosMetrics.isSimulated && (
        <div className="absolute top-2 right-2 px-2 py-1 bg-yellow-500/20 text-yellow-400 text-xs rounded-full">
          Demo Mode
        </div>
      )}
